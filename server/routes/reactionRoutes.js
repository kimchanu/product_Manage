...
router.post("/:id/reaction", async (req, res) => {
    try {
        ...
        if (!user_id || !reaction_type || !["like", "dislike"].includes(reaction_type)) {
            return res.status(400).json({ error: "올바른 사용자 ID와 반응 타입을 입력해 주세요." });
        }
        ...
        try {
            ...
            if (existingReaction.length > 0) {
                ...
                if (oldReaction === reaction_type) {
                    ...
                    return res.json({ message: "반응이 취소되었습니다.", reaction_type: null });
                }
                ...
                await transaction.commit();
                return res.json({ message: "반응이 변경되었습니다.", reaction_type });
            }
            ...
            await transaction.commit();
            return res.json({ message: "반응이 추가되었습니다.", reaction_type });
        } catch (error) {
...
router.get("/:id/reaction/:user_id", async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "반응 상태 조회 중 오류가 발생했습니다." });
    }
});
