...
router.put("/:material_id/:id", async (req, res) => {
    ...
    try {
        ...
        if (id.startsWith('gw_')) {
            ...
            const existingApiItem = await ApiMainProduct.findByPk(gwId);

            if (!existingApiItem) {
                return res.status(404).json({ message: "?대떦 ?낃퀬 湲곕줉??李얠쓣 ???놁뒿?덈떎." });
            }

            await assertPeriodUnlocked({
                businessLocation: business_location,
                department,
                date: existingApiItem.date,
                actionLabel: "수정",
            });
            await assertPeriodUnlocked({
                businessLocation: business_location,
...
router.post("/manual", authMiddleware, async (req, res) => {
    ...
    try {
        await assertPeriodUnlocked({
            businessLocation: defaultValues.business_location,
