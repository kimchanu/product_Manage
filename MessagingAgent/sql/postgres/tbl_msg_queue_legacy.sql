create sequence {SCHM}.{SENDTABLE}_seq increment 1 start 1 maxvalue 999999999 minvalue 1 cycle cache 20;

create table {SCHM}.{SENDTABLE}
( 
mseq int DEFAULT nextval('{SENDTABLE}_seq'), 
msg_type char(1) not null default '1', 
send_type char(1) not null default '1', 
dkey varchar(50) not null default '0', 
dcnt int not null default 0, 
dstaddr varchar(20) not null default '0', 
callback varchar(20) null, 
stat char(1) not null default '0', 
subject varchar(120) null default ' ', 
text_type char(1) not null default '0', 
text varchar(4000) not null default ' ',
text2 varchar(4000) null,
expiretime int null default 86400, 
filecnt    int not null default 0, 
fileloc1   varchar(512) null, 
filesize1  int null, 
fileloc2   varchar(512) null, 
filesize2  int null, 
fileloc3   varchar(512) null, 
filesize3  int null, 
fileloc4   varchar(512) null, 
filesize4  int null, 
fileloc5   varchar(512) null, 
filesize5  int null, 
filecnt_checkup int not null default 0, 
insert_time timestamp null, 
request_time timestamp not null, 
send_time timestamp null, 
report_time timestamp null, 
tcprecv_time timestamp null, 
save_time timestamp null,
telecom varchar(10) null, 
result char(4) null, 
repcnt int not null default 0, 
server_id int null, 
opt_id varchar(20) null, 
opt_cmp  varchar(40) null, 
opt_post  varchar(40) null, 
opt_name varchar(40) null, 
ext_col0 int null, 
ext_col1 varchar(64) null, 
ext_col2 varchar(32) null, 
ext_col3 varchar(32) null,
pseq varchar(10) null,
sender_key varchar(40) null,

k_template_code varchar(30) null,
k_expiretime int null default 180,
k_img_link_url varchar(256) null,
k_next_type int null default 0,

k_button_type  char(2) null,
k_button_name  varchar(56) null,
k_button_url   varchar(256) null,
k_button_url2  varchar(256) null,
k_button2_type char(2) null,
k_button2_name varchar(56) null,
k_button2_url  varchar(256) null,
k_button2_url2 varchar(256) null,
k_button3_type char(2) null,
k_button3_name varchar(56) null,
k_button3_url  varchar(256) null,
k_button3_url2 varchar(256) null,
k_button4_type char(2) null,
k_button4_name varchar(56) null,
k_button4_url  varchar(256) null,
k_button4_url2 varchar(256) null,
k_button5_type char(2) null,
k_button5_name varchar(56) null,
k_button5_url  varchar(256) null,
k_button5_url2 varchar(256) null,

k_at_send_type char(1) default '0',
k_ad_flag char(1) default 'N',

k_attach varchar(4000) null,
k_attach2 varchar(2000) null,

constraint pk_{SENDTABLE} primary key (mseq)
);
