...
function Sidebar({ onSelectDepartment, selectedDepartment }) {
    ...
    return (
        <aside className="w-60 bg-gray-100 h-screen fixed top-[130px] left-0 p-4 overflow-auto shadow-md pb-20">
            {/* 메뉴 네비게이션 섹션 */}
            ...
            <ul className="space-y-2 text-gray-700">
                {departments.map((displayName) => (
                    <li
                        key={displayName}
                        onClick={() => onSelectDepartment && onSelectDepartment(departmentMap[displayName])}
